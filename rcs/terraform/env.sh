...
DEFAULT_HUMAN_FILE=human.plan
DEFAULT_TF_FILE=tf.plan
alias tf="terraform"
alias tf-fmt="tf fmt -recursive"
alias tf-plan="tf plan -no-color -out ${DEFAULT_TF_FILE} > ${DEFAULT_HUMAN_FILE}"
...
alias tf-apply="tf apply ${DEFAULT_TF_FILE}"
